...
export const sign_in = async (req, res) => {
  ...
  try {
    ...
    User.find({ email }, async (err, data) => {
      if (err)
        res
          ...
          .send({
            errMsg:
              "Sorry, the login failed. PLease, try again after sometime.",
          });
      else {
        if (data.length === 0)
          res
            .status(404)
            .send({ errMsg: "The requested account doesn't exist." });
        else {
          const hashedPass = data[0].password;
          const match = await bcrypt.compare(password, hashedPass);
          if (match) res.status(200).send({ result: data[0], token });
          else
            res
              .status(401)
              .send({ errMsg: "Password doesn't match! Try another one." });
        }
      }
    });
  } catch (err) {
    res
      .status(500)
      .send({
        errMsg: "Sorry, the login failed. PLease, try again after sometime.",
      });
  }
};
    ...
    user
      ...
      .catch((err) =>
        res
          .status(403)
          .send({
            msg: "An account linked to this email address already exists",
          })
      );
  } catch (err) {
...
export const token = async (req, res) => {
  ...
  try {
    ...
    User.find({ email }, (err, data) => {
      if (err) res.status(502).send({ errMsg: "Database error." });
      else {
        if (data.length === 0)
